...
- 매 영업일 08:00, KRX 상위 500종목을 7각도(기술·재무·수급·뉴스·분봉·커뮤니티·유튜브)로 채점해 **상위 종목을 텔레그램으로 발송** — 추천 카드에 "왜 이 종목"(뉴스·추세 근거 한 줄)과 "주도 전문가"(가중치×점수 상위)를 함께 표시
- 사용자는 **버튼 한 번**으로 KIS 증권사 API에 매수 주문 — 안전장치(시드 상한·집중 한도·연속 손실·PIN·장 시간 가드) 통과 시만 체결
- 장중 3분 간격 가격 모니터링 → 목표가·손절가 도달 시 즉시 알림
- `/잔고` 에 보유 종목별 **자동매도가(목표·손절) 표시 + 🔴즉시매도 버튼** — 자동매도 트리거 전 수동 청산 가능
- 장마감 후 데이터 수집 — **15:40 당일 분봉**, 16:00 일봉·재무·수급 증분 → 다음 영업일 추천 입력(분봉 expert 포함 7전문가 풀가동)
...
- **Config 노브** — `RECOMMEND_MIN_SCORE` · `AUTO_RECOMMEND_ENABLED` · `MARKET_DOWN_THRESHOLD_PCT` · `REGIME_DEGRADED_*`(급변장 단타 전환 on·점수·시드%) · `BALANCE_CACHE_TTL_SEC`(잔고 캐시 TTL) · `OHLCV_MINUTE_RETENTION_DAYS`(분봉 보존일) · 모드별 TP/SL — 코드 변경 없이 운영 조정
