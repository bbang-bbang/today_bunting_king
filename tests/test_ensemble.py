...
def test_weighted_sum_bunt_3expert(dummy_enriched):
    ...
    # 번트 7-expert 중 tech/fund/flow 만 유효 → 유효 가중치 재정규화 (v4 가중치)
    # (80*0.23 + 60*0.25 + 40*0.13) / (0.23+0.25+0.13) = 38.6 / 0.61 ≈ 63.279
    assert op.ensemble_score == pytest.approx(63.279, abs=0.01)
...
def test_weighted_sum_squeeze_3expert(dummy_enriched):
    ...
    op = scorer.evaluate("X", dummy_enriched)
    # 스퀴즈 (v4): (80*0.26 + 60*0.15 + 40*0.13) / (0.26+0.15+0.13) = 35.0 / 0.54 ≈ 64.815
    assert op.ensemble_score == pytest.approx(64.815, abs=0.01)
...
def test_missing_fundamental_fallback(dummy_enriched):
    ...
    op = scorer.evaluate("X", dummy_enriched)
    assert op.fallback_used
    # 재무 누락 → tech 0.23, flow 0.13 유효 → 재정규화 (v4)
    # (70*0.23 + 50*0.13) / (0.23+0.13) = 22.6 / 0.36 ≈ 62.778
    assert op.ensemble_score == pytest.approx(62.778, abs=0.01)
...
def test_missing_flow_fallback(dummy_enriched):
    ...
    op = scorer.evaluate("X", dummy_enriched)
    assert op.fallback_used
    # flow 누락 → tech 0.23, fund 0.25 유효 (v4)
    # (70*0.23 + 60*0.25) / (0.23+0.25) = 31.1 / 0.48 ≈ 64.792
    assert op.ensemble_score == pytest.approx(64.792, abs=0.01)
...
def test_mode_fit_weighted_average(dummy_enriched):
    ...
    op = scorer.evaluate("X", dummy_enriched)
    # bunt (v4): (0.8*0.23 + 0.6*0.25 + 0.4*0.13) / (0.23+0.25+0.13) = 0.386 / 0.61 ≈ 0.6328
    assert op.mode_fit["bunt"] == pytest.approx(0.6328, abs=0.001)
